...
describe('parseControlCommand: B5-P7 记忆管理命令', () => {
  ...
  it('reveal-path：只认会话 id 与相对路径，越界判定留给 main', async () => {
    const { parseControlCommand, REVEAL_PATH_MAX } = await import('../src/control-model.ts')
    // open-workspace 已随面板上的"打开资源管理器"按钮一起撤掉（#4）：官方右侧
    // Sidebar 有文件树；现在它是未知命令，整条拒绝。
    expect(parseControlCommand({ type: 'open-workspace', sessionId: 's1' })).toBeNull()
  ...
  it('save-global-memory 接受空串清空，拒绝超长与非字符串', async () => {
    ...
    expect(parseControlCommand({ type: 'save-global-memory', content: '', home: '/home', expected: '' }))
      .toEqual({ type: 'save-global-memory', content: '', home: '/home', expected: '' })
