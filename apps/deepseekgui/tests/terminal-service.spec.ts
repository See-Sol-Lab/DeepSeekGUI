...
describe('dsh-wrapper 真实 spawn（argv 注入语义）', () => {
  const wrapper = join(process.cwd(), 'apps', 'deepseekgui', 'src', 'terminal', 'dsh-wrapper.cjs')
...
describe('writeTerminalShims 落盘', () => {
  /** 造一个能被 writeTerminalShims 读到 dsh-wrapper.cjs 的假 moduleDir。 */
  function stage() {
    const rootDir = mkdtempSync(join(tmpdir(), 'shim-write-'))
    const moduleDir = join(rootDir, 'lib')
    mkdirSync(join(rootDir, 'src', 'terminal'), { recursive: true })
    mkdirSync(moduleDir, { recursive: true })
    writeFileSync(join(rootDir, 'src', 'terminal', 'dsh-wrapper.cjs'), '// wrapper source\n')
    return { rootDir, moduleDir, binDir: join(rootDir, 'bin') }
  }

  it('把 wrapper 与三个转发脚本写进指定目录，目录不存在就建出来', () => {
    const { rootDir, moduleDir, binDir } = stage()
    try {
      const returned = writeTerminalShims({
        binDir, packaged: false, root: rootDir, resourcesPath: join(rootDir, 'res'),
        moduleDir, env: { npm_node_execpath: '/usr/bin/node', npm_execpath: '/x/pnpm.cjs' },
        activeProfile: 'default', platform: 'win32',
      })
      expect(returned).toBe(binDir)
      expect(readFileSync(join(binDir, 'dsh-wrapper.cjs'), 'utf8')).toBe('// wrapper source\n')
      for (const name of ['node.cmd', 'dsh.cmd', 'pnpm.cmd']) {
        expect(existsSync(join(binDir, name))).toBe(true)
      }
      // shim 指向写出的真实 wrapper 文件，不指回源树。
      expect(readFileSync(join(binDir, 'dsh.cmd'), 'utf8')).toContain(join(binDir, 'dsh-wrapper.cjs'))
    } finally {
      rmSync(rootDir, { recursive: true, force: true })
    }
  })

  it('npm_execpath 指向 npm 时不当作 pnpm，改写明确的提示 shim（P7-J）', () => {
    // npm 与 pnpm 都注入 npm_execpath。认错的后果是终端里敲 pnpm 实际跑 npm，
    // 那正是 P7-J 那盏稳定红灯的成因，所以这里按文件名严格判。
    const { rootDir, moduleDir, binDir } = stage()
    try {
      writeTerminalShims({
        binDir, packaged: false, root: rootDir, resourcesPath: join(rootDir, 'res'),
        moduleDir, env: { npm_execpath: '/n/npm-cli.js' },
        activeProfile: 'default', platform: 'win32',
      })
      const written = readFileSync(join(binDir, 'pnpm.cmd'), 'utf8')
      expect(written).toContain('dev 模式下请经 pnpm script 启动')
      expect(written).not.toContain('npm-cli.js')
    } finally {
      rmSync(rootDir, { recursive: true, force: true })
    }
  })

  it('认得 Windows 路径分隔符下的 pnpm 入口', () => {
    const { rootDir, moduleDir, binDir } = stage()
    try {
      writeTerminalShims({
        binDir, packaged: false, root: rootDir, resourcesPath: join(rootDir, 'res'),
        moduleDir, env: { npm_execpath: 'C:\\tools\\pnpm\\bin\\pnpm.cjs' },
        activeProfile: 'default', platform: 'win32',
      })
      // 认出来了就不该写提示 shim，而是真的转发到那个入口。
      const written = readFileSync(join(binDir, 'pnpm.cmd'), 'utf8')
      expect(written).toContain('pnpm.cjs')
      expect(written).not.toContain('dev 模式下请经')
    } finally {
      rmSync(rootDir, { recursive: true, force: true })
    }
  })

  it('POSIX 形态写无扩展名脚本并补执行位', () => {
    const { rootDir, moduleDir, binDir } = stage()
    try {
      writeTerminalShims({
        binDir, packaged: false, root: rootDir, resourcesPath: join(rootDir, 'res'),
        moduleDir, env: {}, activeProfile: 'default', platform: 'linux',
      })
      for (const name of ['node', 'dsh', 'pnpm']) {
        const target = join(binDir, name)
        expect(existsSync(target)).toBe(true)
        if (process.platform !== 'win32') expect(statSync(target).mode & 0o111).not.toBe(0)
      }
      expect(existsSync(join(binDir, 'node.cmd'))).toBe(false)
    } finally {
      rmSync(rootDir, { recursive: true, force: true })
    }
  })
