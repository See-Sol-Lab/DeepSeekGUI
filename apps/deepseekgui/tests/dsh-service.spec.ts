...
// stubEnv 的还原交给 vitest，不再各写一份 try/finally。
...
describe('resolveDshLaunch', () => {
  ...
  it('开发态未注入 npm_node_execpath 时回退到 PATH 中的 node', () => {
    vi.stubEnv('npm_node_execpath', undefined)
...
describe('内置浏览器 overlay 与 profile bundles 互斥', () => {
  ...
  function stageBrowserRepo(): string {
    ...
    const dir = join(root, 'apps', 'deepseekgui', 'browser-plugin')
...
describe('内置插件的定位规则（五个插件同一张表）', () => {
  /**
   * overlay 走 launcher 层而不是 `dsh plugin add`：合成顺序里 --patch 落在
   * 最后，只影响 DeepSeekGUI 启动的这一轮，用户 profile 的清单一个字节都不改。
   *
   * 这组用例此前只覆盖 theme 与 workbench 两个插件，各写一遍；定位规则收进
   * BUNDLED_PLUGINS 之后改成遍历整张表——picker 的「只随包发行」与 browser 的
   * 「开发态 overlay 另有其名」于是第一次被自动验证到。
   */
  const PACKAGED = { packaged: true, resourcesPath: 'C:\app\resources' }
  const DEV = { packaged: false, root: 'R:\repo' }

  for (const [key, plugin] of Object.entries(BUNDLED_PLUGINS)) {
    it(`${key}：打包态指向 DSH 运行时目录内（那个 Node 进程读不到 asar）`, () => {
      expect(bundledPluginDir(plugin, PACKAGED))
        .toBe(join('C:\app\resources', 'dsh', 'node_modules', ...plugin.pkg.split('/')))
      expect(bundledPatchFile(plugin, PACKAGED))
        .toBe(join('C:\app\resources', 'dsh', plugin.overlay[1]))
    })

    it(`${key}：开发态${plugin.devDir === null ? '不存在（刻意）' : '指向仓库内的插件目录'}`, () => {
      if (plugin.devDir === null) {
        // picker 的缺陷只在打包态存在，开发态保持官方实现不动。
        expect(bundledPluginDir(plugin, DEV)).toBeUndefined()
        expect(bundledPatchFile(plugin, DEV)).toBeUndefined()
        return
      }
      expect(bundledPluginDir(plugin, DEV)).toBe(join('R:\repo', 'apps', 'deepseekgui', plugin.devDir))
      expect(bundledPatchFile(plugin, DEV))
        .toBe(join('R:\repo', 'apps', 'deepseekgui', plugin.devDir, plugin.overlay[0]))
    })

    it(`${key}：缺少定位信息时不产生 overlay——宁可少个插件，也不能让 Harness 起不来`, () => {
      expect(bundledPluginDir(plugin, { packaged: true })).toBeUndefined()
      expect(bundledPluginDir(plugin, { packaged: false })).toBeUndefined()
      expect(bundledPatchFile(plugin, { packaged: true })).toBeUndefined()
      expect(bundledPatchFile(plugin, { packaged: false })).toBeUndefined()
    })
  }

  it('browser 是唯一开发态与运行时 overlay 不同名的：开发态用插件自带的那份', () => {
    expect(BUNDLED_PLUGINS.browser.overlay[0]).toBe('cordis.patch.yml')
    expect(BUNDLED_PLUGINS.browser.overlay[1]).toBe(BROWSER_PATCH_FILENAME)
    for (const plugin of Object.values(BUNDLED_PLUGINS)) {
      if (plugin.pkg === BUNDLED_PLUGINS.browser.pkg) continue
      expect(plugin.overlay[0]).toBe(plugin.overlay[1])
    }
  })
  ...
  it('插件可解析时建立 fallback 链接，并把 overlay 排在 dsh 自己的选项区内', () => {
    ...
    const pluginDir = join(root, 'apps', 'deepseekgui', 'theme-plugin')
...
describe('Workbench overlay（B3-P1）与模块 fallback', () => {
  it('插件可解析时 overlay 排在 dsh 选项区内，链接落在安装级 fallback', () => {
    const root = mkdtempSync(join(tmpdir(), 'deepseekgui-root-'))
    const home = mkdtempSync(join(tmpdir(), 'deepseekgui-home-'))
    const pluginDir = join(root, 'apps', 'deepseekgui', 'workbench-plugin')
    mkdirSync(pluginDir, { recursive: true })
  ...
  it('Compatibility View：四条产品 overlay 全可解析也一条都不带', () => {
    ...
    const home = mkdtempSync(join(tmpdir(), 'deepseekgui-home-'))
    const staged = [
      { devDir: 'workbench-plugin', patch: WORKBENCH_PATCH_FILENAME },
    ...
    for (const { devDir, patch } of staged) {
      ...
      mkdirSync(pluginDir, { recursive: true })
      writeFileSync(join(pluginDir, patch), '- insert: []\n')
    }

    const compatible = resolveDshLaunch({
      packaged: false,
      root,
      nodeExecutable: 'node',
      profile: 'web',
    ...
    const patches = compatible.args.flatMap((arg, index) => arg === '--patch' ? [compatible.args[index + 1] ?? ''] : [])
    expect(patches).toHaveLength(0)

    // 同一棵树在 Workbench 模式下确实带得出 overlay，否则上面的 0 条可能只是
    // 插件没铺对，而不是 Compatibility View 起了作用。
    const workbench = resolveDshLaunch({
      packaged: false,
      root,
      nodeExecutable: 'node',
      profile: 'web',
    ...
    const workbenchPatches = workbench.args.flatMap((arg, index) => arg === '--patch' ? [workbench.args[index + 1] ?? ''] : [])
    expect(workbenchPatches.length).toBeGreaterThan(0)
