...
function makeCtx(overrides: Record<string, unknown> = {}) {
  ...
  const git = {
    ...
    pushPreview: vi.fn(async () => ({ sourceOid: 'sha-2', destinationToken: 'destination-proof', remote: { name: 'origin', fetchUrl: 'https://example.invalid/repo.git', pushUrl: 'https://example.invalid/push.git' }, localBranch: 'main', remoteBranch: 'main', aheadCommits: [], remoteRefExists: true })),
...
describe('write gates', () => {
  ...
  it('push requests approval before the external write', async () => {
    ...
    expect(git.push).toHaveBeenCalledWith('C:\\repo', 'origin', 'main', 'main', { sourceOid: 'sha-2', destinationToken: 'destination-proof' })
