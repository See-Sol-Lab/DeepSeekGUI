...
export function validatePluginRequest(request: PluginOperationRequest): string | null {
  ...
  if (request.spec.startsWith('-')) return 'spec 必须是插件包名或路径，不能是命令行选项'
...
export function pluginConfirmText(input: PluginConfirmInput): PluginConfirmText {
  ...
  if (input.homeKind === 'existing') {
    lines.push('', zh ? '这次操作会修改你选择的现有 Harness Profile。' : 'This operation modifies the existing Harness Profile you selected.')
  }
  return {
...
/** plugin 操作的流式输出上限：行数。超出丢最早。 */
export const PLUGIN_OUTPUT_MAX_LINES = 300
/** plugin 操作的流式输出上限：字节。超出丢最早，绝不击穿内存。 */
export const PLUGIN_OUTPUT_MAX_BYTES = 64 * 1024

/**
 * 往已有输出后面追加一段文本，并把结果压回上限之内。
 *
 * 纯函数：调用方拿返回值替换自己的视图。空行不入账（子进程的换行噪声
 * 不该占预算），行数与字节两条上限任一越界都从最早的行开始丢。传入的
 * 文本必须已经脱敏——这里只管长度，不管内容。
 * @param existing - 现有输出行。
 * @param text - 新到的一段文本，可含多行。
 * @returns 追加并截断后的新数组；入参不被修改。
 */
export function appendPluginOutput(existing: readonly string[], text: string): string[] {
