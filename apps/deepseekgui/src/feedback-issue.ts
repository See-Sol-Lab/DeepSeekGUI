 ...
 */
/**
 * The prompt the desktop sends to the hidden feedback-triage session (#15).
 * Materials only: the user's words and the redacted diagnostics. The reply
 * is asked to end with a `**标题：**` / `**Title:**` line so {@link issueTitle}
 * can lift it.
 * @param userText - what the user typed in the feedback panel.
 * @param diagnostics - the redacted, possibly user-edited diagnostics bundle.
 * @param zh - whether the interface is Chinese.
 * @returns the prompt text.
 */
export function feedbackTriagePrompt(userText: string, diagnostics: string, zh = true): string {
  return zh
    ? [
      '用户在 DeepSeekGUI 桌面端的"BUG 诊断与反馈"面板里描述了一个问题。下面是用户的描述和一份自动脱敏的诊断包，请只根据这些材料排查，不要调用任何工具，也不要改任何文件。',
      '',
      '请按这个顺序回答，用中文，直接给结论不要客套：',
      '1. 最可能的原因（一两句）。',
      '2. 用户现在能自己做的一两步。',
      '3. 一段可以直接贴进 GitHub issue 的英文摘要（三到五句）。',
      '4. 最后单独一行写 `**标题：** ` 加一句不超过 60 字的 issue 标题。',
      '',
      '## 用户描述',
      userText,
      '',
    ...
    : [
      'A user described a problem in the DeepSeekGUI desktop app\'s "Bug diagnostics & feedback" panel. Below are their words and an automatically redacted diagnostics bundle. Triage from these materials only; do not call any tool and do not change any file.',
      '',
      'Answer in this order, plainly:',
      '1. The most likely cause (one or two sentences).',
      '2. One or two steps the user can take right now.',
      '3. A three-to-five-sentence summary ready to paste into a GitHub issue.',
      '4. Finally, on its own line, `**Title:** ` followed by an issue title of at most 80 characters.',
      '',
      '## User description',
      userText,
      '',
...
export function githubNewIssueUrl(title: string, body = '', zh = true): string {
  ...
  if (body === '') {
    return `${base}?template=bug_report.md&labels=user-feedback&title=${encodeURIComponent(title.toWellFormed())}`
  }
  let encoded = encodeURIComponent(body.toWellFormed())
  if (encoded.length > 6000) {
    ...
    while (encoded.length > 6000 && cut.length > 0) {
      ...
      encoded = encodeURIComponent((zh
        ...
        : `${cut}\n\n… (The body was truncated for the URL. Paste the complete content from the clipboard to replace it.)`).toWellFormed())
    }
  }
  return `${base}?labels=user-feedback&title=${encodeURIComponent(title.toWellFormed())}&body=${encoded}`
}
