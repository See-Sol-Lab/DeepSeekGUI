...
export interface ControlDispatchDeps {
  ...
  updateInstall: () => void | Promise<void>
  /** Diagnostics：打开日志文件夹。 */
  openLogFolder: () => void
  /** Diagnostics：导出 bundle（本地目录，绝不上传）。 */
  exportDiagnostics: () => void
  /** 权限模式切换出口（sandbox / full-access；确认与官方写入由 main 接线）。 */
  setPermissionMode: (mode: 'sandbox' | 'full-access') => void | Promise<void>
  ...
  /** 收起内置浏览器 pane（#13 与官方 Sidebar 互斥；已收起时 no-op）。 */
  browserPaneHide: () => void
  /**
   * 切到 Compatibility View（B3-P2）：不带 Workbench 插件的官方界面。
   * 确认与重启由 main 接线（与 restart-harness 同一控制器路径）。
   */
  openCompatibilityView: () => void | Promise<void>
  ...
  return async (command) => {
    switch (command.type) {
      ...
      case 'set-permission-mode':
        await deps.setPermissionMode(command.mode)
        return
      case 'open-feedback':
        deps.openFeedback()
        return
      ...
      case 'browser-pane-toggle':
        ...
        return
      case 'browser-pane-hide':
        deps.browserPaneHide()
        return
      case 'open-compatibility-view':
        await deps.openCompatibilityView()
        return
      case 'open-workbench':
