 ...
 * - 绝不请求用户 personal token、绝不把仓库凭据打包；没有覆盖文件时用内置
 *   公开通道，覆盖文件损坏/非 HTTPS/带凭据时明确 unconfigured——Manual Check
 *   显示"当前未配置公开更新通道"，background check 安静结束。
 ...
 * 按当前平台挑选可安装资产。
 *
 * manifest 没有平台字段，资产的身份写在 filename 后缀里（发行物事实：
 * Windows 是 NSIS `*.exe`，Linux 是 `*.AppImage`）。没有匹配项就是明确的
 * "这个平台没有可用资产"，绝不退回 `assets[0]` 去下载另一个平台的包。
 * @param assets - manifest 的资产列表。
 * @param platform - `process.platform` 的取值。
 * @returns 选中的资产，或平台无资产的事实。
 */
export function selectPlatformAsset(
  assets: readonly UpdateAsset[],
  platform: string,
): { ok: true; asset: UpdateAsset } | { ok: false; reason: 'no-platform-asset' } {
  const suffix = platform === 'win32' ? '.exe' : platform === 'linux' ? '.AppImage' : null
  if (suffix === null) return { ok: false, reason: 'no-platform-asset' }
  const asset = assets.find(candidate => candidate.filename.toLowerCase().endsWith(suffix.toLowerCase()))
  return asset === undefined ? { ok: false, reason: 'no-platform-asset' } : { ok: true, asset }
}

/**
 * 缓存文件命名：下载中的临时文件与校验通过的文件必须分名。
 *
 * 只有校验通过的最终名字才能成为 runner 可执行的对象；一个被截断或
 * digest 不符的下载永远停在 `.partial` 上，界面与安装路径都看不到它。
 * @param dir - 更新缓存目录。
 * @param filename - 经 sanitize 的资产文件名。
 * @returns 临时路径与最终路径。
 */
export function updateCachePaths(dir: string, filename: string): { partial: string; verified: string } {
  const final = win32.basename(filename)
  return { partial: `${dir}/${final}.partial`, verified: `${dir}/${final}` }
}

/**
 * 是否应为一个刚发现的可用更新启动后台下载。
 *
 * 自动下载只在用户没有关闭它、没有正在进行的下载或检查、且缓存里还
 * 没有这个版本的已验证产物时启动。同一版本因此只有一个下载任务。
 * @param input - 开关、当前面板状态、进行中的下载与已有产物。
 * @returns true 表示可以启动。
 */
export function shouldAutoDownloadUpdate(input: {
  ...
  if (!input.autoDownload || input.busy) return false
  if (input.state === 'downloading' || input.state === 'checking') return false
  if (input.verifiedVersion === input.latestVersion) return false
  return true
}
...
function parseUpdateAsset(raw: unknown, where: string, zh = true): UpdateAsset {
  ...
  if (typeof size !== 'number' || !Number.isSafeInteger(size) || size <= 0 || size > UPDATE_SIZE_LIMIT) {
    throw new UpdateManifestError(zh ? `${where}.size: 必须是 1 到 ${UPDATE_SIZE_LIMIT} 之间的整数字节数` : `${where}.size: must be an integer byte count from 1 to ${UPDATE_SIZE_LIMIT}`)
  }
  ...
  if (sanitizeAssetFilename(filename) !== filename) throw new UpdateManifestError('Invalid update asset filename')
...
export function parseUpdateManifest(text: string, zh = true): UpdateManifest {
  ...
  return {
    latestVersion,
    // 展示文本有界：feed 是外部输入，超长 release notes 只截断到上限，
    // 绝不整段进面板或诊断包。
    releaseNotes: releaseNotes.slice(0, UPDATE_RELEASE_NOTES_MAX),
...
export function sanitizeAssetFilename(raw: string): string | null {
  ...
  if (base.endsWith('.') || /^(?:con|prn|aux|nul|com[1-9]|lpt[1-9])(?:\.|$)/iu.test(base)) return null
  return base
}
...
async function attemptDownload(
  ...
  return new Promise((resolve, reject) => {
    let total = 0
    // `as boolean`：fail/finish 在闭包里改它，get 的同步回调可能在下面
    // 那句 `if (settled)` 之前就已置位——不加宽，TS 会把它收窄成永远 false。
    ...
    try {
      requestRef.current = get(url, (response) => {
        ...
        response.on('data', (chunk: unknown) => {
          ...
          if (bytes.length > 0) armIdleTimeout()
          total += bytes.length
          if (total > maxBytes) {
      ...
      fail(new UpdateDownloadError(String(error instanceof Error ? error.message : error)))
      return
    }
