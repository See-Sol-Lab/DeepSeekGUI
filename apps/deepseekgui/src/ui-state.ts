/**
 * Desktop UI state：窗口几何、恢复提示确认与面板偏好。
...
export interface DesktopUiStateV1 {
  ...
  /**
   * 发现更新后是否后台自动下载（B6-P6）。默认开：用户装好就能收到新版本，
...
export function parseUiState(content: string, zh = true): DesktopUiStateV1 {
  ...
  rejectUnknownKeys(
    raw,
    // themePreference 仍在名单里只是为了容忍老文件：主题的唯一真源是官方
  ...
  // autoDownloadUpdate 与 terminalBounds 同为版本 2 存续期内后加的字段：
  // 缺失按默认（开）宽容一次，老文件不会因此整份回退；给出但非法仍然拒绝。
