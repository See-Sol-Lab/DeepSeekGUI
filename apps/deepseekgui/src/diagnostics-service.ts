 ...
 * 除末尾两个 IO 函数（读日志家族、把 bundle 落盘）外全是纯逻辑；那两个
 * 也只碰 fs，同样不依赖 Electron，便于单元测试。
 * @module @see-sol-lab/deepseekgui/diagnostics-service
 */

import { mkdirSync, readFileSync, writeFileSync } from 'node:fs'
import { basename, dirname, join } from 'node:path'
import { logFamilyNames } from './dsh-service.ts'
import { maskWindowsLiterals } from './redact.ts'
import type { DeepSeekGUIVersionInfo } from './version-info.ts'

/**
 * 主进程自己的日志文件名（userData 下）。打包态 Electron 主进程的 stderr
 * 没人接，`console.error` 一律消失——桌面壳自己的失败此前只剩 events.md 与
 * 诊断包里的转述，没有原始记录可翻（2026-09-11 人工测试 #19，DS 报的）。
 * 与 dsh-service.log 同一套写入器：限长、轮转、脱敏。
 */
export const MAIN_LOG_FILENAME = 'deepseekgui-main.log'
...
/**
 * 收集日志家族：current 加上全部轮转历史，逐份脱敏。
 *
 * 单个文件读不动不中断整体——诊断包的价值在于尽量多，不在于全；少一份
 * 轮转日志好过一份都拿不到。日志路径未知时返回空数组，空日志的
 * unavailable 占位由 {@link assembleDiagnosticsBundle} 统一生成。
 * @param logPath - 当前服务日志的绝对路径；undefined = 还没有日志。
 * @param redact - 脱敏函数，每份内容进 bundle 前都要过它。
 * @param onEachFile - 每读完一份调用一次；headless 导出用它检查总超时。
 * @returns 已脱敏的日志副本，按轮转顺序。
 */
export function collectLogFamily(
  ...
  for (const name of logFamilyNames(dir, basename(logPath))) {
    try {
      entries.push({ name, content: redact(readFileSync(join(dir, name), 'utf8')), source: join(dir, name) })
    } catch {
...
/**
 * 把组装好的 bundle 落盘到 `<userData>/diagnostics/diagnostics-<时间戳>/`。
 *
 * 时间戳里的 `:` 与 `.` 换成 `-`：Windows 的文件名不接受冒号，而带点的
 * 目录名在打包与压缩链路上容易被当成扩展名。
 * @param userDataDir - userData 目录。
 * @param files - 文件名到内容的映射（已经过 allowlist 过滤）；crash dump
 * 一类证据是二进制，所以值可以是 Buffer。
 * @param at - 导出时刻。
 * @returns 写入的目录绝对路径。
 */
export function writeDiagnosticsBundle(
  ...
  const dir = join(userDataDir, 'diagnostics', `diagnostics-${at.toISOString().replace(/[:.]/gu, '-')}`)
