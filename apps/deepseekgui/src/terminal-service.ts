 ...
 * 除 writeTerminalShims 外全部纯函数；那一个把 shim 落盘，依赖（目录、
 * 形态、环境、平台）同样全部由调用方注入，所以整个模块仍不依赖 Electron，
 * 便于单元测试。
...
function batchValue(value: string): string {
  if (/["\r\n]/u.test(value)) throw new Error('terminal path contains an unsupported batch character')
...
function shimCmd(
  ...
  const head = ['@echo off', 'setlocal DisableDelayedExpansion', ...envSets]
  if (prefixArgs.length === 0) {
    head.push(`"${batchValue(executable)}" %*`)
  } else {
    head.push(`"${batchValue(executable)}" ${prefixArgs.map(arg => `"${batchValue(arg)}"`).join(' ')} %*`)
  }
  return `${head.join('\r\n')}\r\n`
}
...
function dshShimCmd(facts: ShimRuntimeFacts): string {
  const prefix = facts.nodePrefixArgs.map(arg => `"${batchValue(arg)}" `).join('')
  return [
    '@echo off',
    'setlocal DisableDelayedExpansion',
    'set "ELECTRON_RUN_AS_NODE=1"',
    `set "DEEPSEEKGUI_WRAPPER_EXE=${batchValue(facts.nodeExecutable)}"`,
    `set "DEEPSEEKGUI_WRAPPER_DSH_BIN=${batchValue(facts.dshBin)}"`,
    `set "DEEPSEEKGUI_WRAPPER_NODE_ARGS=${JSON.stringify(facts.dshNodeArgs)}"`,
    `set "DEEPSEEKGUI_ACTIVE_PROFILE=${batchValue(facts.activeProfile)}"`,
    `"${batchValue(facts.nodeExecutable)}" ${prefix}"${batchValue(facts.dshWrapperPath)}" %*`,
    '',
  ].join('\r\n')
}
...
function shimSh(
  ...
  const prefix = prefixArgs.map(arg => `${shellQuote(arg)} `).join('')
  return `${['#!/bin/sh', ...envExports, `exec ${shellQuote(executable)} ${prefix}"$@"`].join('\n')}\n`
}
...
function dshShimSh(facts: ShimRuntimeFacts): string {
  const prefix = facts.nodePrefixArgs.map(arg => `${shellQuote(arg)} `).join('')
  return [
    '#!/bin/sh',
    'export ELECTRON_RUN_AS_NODE=1',
    `export DEEPSEEKGUI_WRAPPER_EXE=${shellQuote(facts.nodeExecutable)}`,
    `export DEEPSEEKGUI_WRAPPER_DSH_BIN=${shellQuote(facts.dshBin)}`,
    `export DEEPSEEKGUI_WRAPPER_NODE_ARGS=${shellQuote(JSON.stringify(facts.dshNodeArgs))}`,
    `export DEEPSEEKGUI_ACTIVE_PROFILE=${shellQuote(facts.activeProfile)}`,
    `exec ${shellQuote(facts.nodeExecutable)} ${prefix}${shellQuote(facts.dshWrapperPath)} "$@"`,
    '',
  ].join('\n')
}
...
/** 把 shim 落盘要用的全部事实——模块自己不去问环境，一律由调用方给。 */
export interface TerminalShimTargets {
  /** shim 目录（应用私有、app-owned）；不存在会被建出来。 */
  readonly binDir: string
  /** 打包态。 */
  readonly packaged: boolean
  /** 仓库根：dev 形态据此定位 cli 入口。 */
  readonly root: string
  /** 打包资源根：packaged 形态据此定位 dsh 与 pnpm。 */
  readonly resourcesPath: string
  /** main 模块所在目录；dsh-wrapper.cjs 以它为锚。 */
  readonly moduleDir: string
  /** 进程环境：dev 形态从中读 npm_node_execpath 与 npm_execpath。 */
  readonly env: NodeJS.ProcessEnv
  /** 当前 Profile。 */
  readonly activeProfile: string
  /** 目标平台：win32 写 .cmd，其余写无扩展名脚本并补执行位。 */
  readonly platform: NodeJS.Platform
}

/**
 * 生成应用私有 shim 目录：node/dsh/pnpm 三个转发脚本只指向当前 exact
 * executable，不下载 Runtime、不猜测系统安装。每次打开终端时重写（内容随
 * execPath / active Profile 变化）。该目录只 prepend 给新开的 terminal
 * process，绝不写系统/用户 PATH、注册表或 shell 配置，绝不污染父环境。
 * @param targets - 落盘所需的全部事实。
 * @returns shim 目录绝对路径。
 */
export function writeTerminalShims(targets: TerminalShimTargets): string {
  const { binDir, packaged, root, resourcesPath, moduleDir, env, activeProfile } = targets
  mkdirSync(binDir, { recursive: true })
  const devNode = env.npm_node_execpath ?? 'node'
  ...
  const injectedExecpath = env.npm_execpath
  const devPnpm = injectedExecpath !== undefined && /(^|[\\/])pnpm(\.cjs|\.mjs|\.js)?$/u.test(injectedExecpath)
    ? injectedExecpath
    : undefined
  // dsh wrapper：main 进程（fs 已带 asar 补丁）从与 chrome/terminal 资产
  // 相同的锚（moduleDir → ../src/terminal/）读出内容，连同三个转发脚本一起
  // 写进 shim 目录——shim 指向写出的真实文件。dev/packaged 同一路径处理，
  // 消除形态分叉；纯 Node 模式的 shim 绝不依赖读 asar。
  const wrapperTarget = join(binDir, 'dsh-wrapper.cjs')
  writeFileSync(wrapperTarget, readFileSync(join(moduleDir, '..', 'src', 'terminal', 'dsh-wrapper.cjs'), 'utf8'))
  const facts: ShimRuntimeFacts = {
    ...
    dshBin: packaged
      ? join(resourcesPath, 'dsh', 'node_modules', '@deepseek-ai', 'dsh', 'lib', 'bin.js')
      : join(root, 'apps', 'cli', 'src', 'bin.ts'),
    dshNodeArgs: packaged ? ['--expose-internals'] : ['--import', 'tsx/esm'],
    pnpmArgs: packaged
      ? ['--expose-internals', join(resourcesPath, 'dsh', 'node_modules', 'pnpm', 'bin', 'pnpm.cjs')]
      : devPnpm === undefined ? [] : [devPnpm],
    activeProfile,
  }
  // Windows 写 .cmd，POSIX 写无扩展名脚本并补执行位——同一转发协议，
  // 两种宿主 shell 的字面量形态。
  const windows = targets.platform === 'win32'
