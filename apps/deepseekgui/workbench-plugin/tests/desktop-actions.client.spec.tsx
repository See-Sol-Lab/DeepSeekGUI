...
describe('DesktopActions', () => {
  ...
  it('paints nothing visible (D1) while the poll runs and the phase rides a data attribute', async () => {
    ...
    await vi.waitFor(() => { expect(phaseOf(container)).toBe('running') })
    expect(container.querySelector('button')).toBeNull()
    expect(container.textContent).toBe('')
    expect(bridge.model).toHaveBeenCalled()
  })

  it('collapses the Sidebar once on the pane closed → open edge, not on every poll (#13)', async () => {
    const onBrowserPaneOpen = vi.fn()
    const model = vi.fn()
      .mockResolvedValueOnce({ changed: true, revision: 1, model: { ...runningModel, browserPane: { present: true, open: false } } })
      .mockResolvedValueOnce({ changed: true, revision: 2, model: { ...runningModel, browserPane: { present: true, open: true } } })
      .mockResolvedValue({ changed: true, revision: 3, model: { ...runningModel, browserPane: { present: true, open: true } } })
    const bridge = bridgeStub({ model })
    vi.useFakeTimers()
    try {
      render(<DesktopActions wide t={t as never} bridge={bridge} openSession={openSession} onBrowserPaneOpen={onBrowserPaneOpen} />)
      await vi.advanceTimersByTimeAsync(0)
      expect(onBrowserPaneOpen).not.toHaveBeenCalled()
      await vi.advanceTimersByTimeAsync(2_000)
      expect(onBrowserPaneOpen).toHaveBeenCalledTimes(1)
  ...
  it('records the bridge error state when the model fetch fails', async () => {
    ...
    const { container } = render(<DesktopActions wide t={t as never} bridge={bridge} openSession={openSession} />)
    await vi.waitFor(() => { expect(phaseOf(container)).toBe('bridge-error') })
  })
})
...
describe('DesktopActions revision gating (P9-2)', () => {
  it('first fetch is unconditional, idle ticks send since, changed:false leaves state alone, failure recovers with a full fetch', async () => {
    ...
    try {
      ...
      expect(phaseOf(container)).toBe('running')
      ...
      await vi.waitFor(() => { expect(phaseOf(container)).toBe('recovered') })
    } finally {
      vi.useRealTimers()
    }
  })
