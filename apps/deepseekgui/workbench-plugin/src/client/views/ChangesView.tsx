...
export function ChangesView({ inspector, bridge, sessionId, t }: ViewProps) {
  ...
  const reveal = (path: string): void => {
    ...
    // Git paths are repository-relative; the desktop resolves against the
    // session cwd, so the reveal must carry the joined absolute path while
    // the row keeps the Git spelling. The desktop re-checks containment.
    void bridge.run({ type: 'reveal-path', sessionId, path: absolutePath(root, path) }).catch(() => undefined)
  }
  ...
  return (
    <section style={view} aria-label={t('view.changes')}>
      ...
      {repository !== undefined && (
        <>
          <div style={caption}>{t('changes.branch')} {repository.status.head.name || repository.status.head.kind}</div>
          {repository.status.clean && <p style={{ margin: 0 }}>{t('changes.clean')}</p>}
          {GROUPS.map((kind) => {
            const entries = repository.status.entries.flatMap(entry => entry.kind === kind ? [entry]
              : kind === 'unstaged' && entry.kind === 'staged' && entry.y !== '.' ? [{ ...entry, kind: 'unstaged' as const }] : [])
