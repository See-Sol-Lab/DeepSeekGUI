 ...
 *
 * Two stacked sections since 2026-09-11 (manual test #5): parallel work trees
 * first (a placeholder when there are none), the repository below. The
 * overview read already carries both, so one refresh serves both.
 */
...
import { WorktreeSection } from './WorktreeView.tsx'

export function GitView({ inspector, sessionId, t, useConversation }: ViewProps & Pick<ConvViewProps, 'useConversation'>) {
  ...
  return (
    <section style={view} aria-label={t('view.git')}>
      ...
      {overview !== undefined && (
        <>
          <div style={sectionTitle}>{t('view.worktree')}</div>
          <WorktreeSection worktrees={overview.worktrees} t={t} />
          <div style={sectionTitle}>{t('view.git')}</div>
          <div>
