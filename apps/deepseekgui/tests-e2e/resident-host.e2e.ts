  ...
  it('G6：fresh Managed Home 首次写入英文 locale 后，Chrome 状态胶囊同步变为英文', async () => {
    ...
    try {
      ...
      try {
        ...
        await waitForCompMount(app)
        await waitLauncherState(temp)
        // 胶囊文案按 view-model 的 D2 决定：默认 profile `web` 是官方 profile
        // 名、不是「网页版」，故意不进胶囊（只有非默认 profile 才带 ` · name`）。
        // 该行为由 chrome-view-model.spec.ts 的 pillView 用例钉住
        // （web → 运行中，lab → 运行中 · lab），所以本用例的预期是「运行中」。
        await expect.poll(
          () => evalInView<string>(app, CHROME_URL_PREFIX, "document.getElementById('status-text')?.textContent ?? ''"),
          { timeout: 15_000 },
        ).toBe('运行中')
