...
describe('git card projection', () => {
  ...
  it('reads entries and counts from the tool projection', () => {
    ...
    expect(view.entries.map(e => e.path)).toEqual(['src/a.ts', 'src/b.ts', 'README.md'])
    expect(view.summary).toBe('3 changed path(s), on b5')
  })

  it('narrows an unknown entry state to unstaged', () => {
    const view = gitSummary('git_status', model({
      kind: 'git-status', clean: false, head: 'on x', counts: { total: 1 },
      entries: [{ state: 'weird', path: 'f.txt', origPath: null }],
    }))
    expect(view.entries[0]?.state).toBe('unstaged')
  })
