...
import { WorktreeView, overlappingPaths } from '../src/client/views/WorktreeView.tsx'
...
describe('ChangesView', () => {
  it('groups changed paths, opens a patch, and offers only reveal / copy / open-workspace', async () => {
    ...
    expect(bridge.run).toHaveBeenCalledWith({ type: 'reveal-path', sessionId, path: 'src/a.ts' })
    ...
    fireEvent.click(screen.getByText('打开资源管理器'))
    expect(bridge.run).toHaveBeenCalledWith({ type: 'open-workspace', sessionId })
...
describe('WorktreeView', () => {
  ...
  it('says only the main tree exists, else lists trees with the current one and overlap warnings', async () => {
    ...
    const first = render(<WorktreeView inspector={only} bridge={null} sessionId={sessionId} submitInstruction={submitInstruction} t={t} />)
    ...
    render(<WorktreeView inspector={two} bridge={null} sessionId={sessionId} submitInstruction={submitInstruction} t={t} />)
...
describe('MemoryView', () => {
  it('renders the project memory as Markdown, offers open / ask / generate-AGENTS only, and never a save', async () => {
    ...
    // One file-manager button only: the generic open-workspace is left off here.
    expect(screen.queryByText(zh['common.openWorkspace'])).toBeNull()
  })
