...
import { MarkdownReader } from './markdown.tsx'
 ...
 */
export const MEMORY_VIEW_MAX_CHARS = 20_000
...
export function MemoryView({ inspector, bridge, sessionId, t, submitInstruction }: ViewProps) {
  ...
  const hasAgents = agentsCreated || memory?.agents === true
  const desktop = bridge !== null && sessionId !== undefined
  return (
    <section style={view} aria-label={t('view.memory')}>
      <Toolbar t={t} refresh={read.refresh} bridge={bridge} sessionId={sessionId} workspace={false}>
        {desktop && (
      ...
      {memory !== undefined && (memory.text === null || memory.text.trim() === ''
        ...
        : (
          <div style={reader} data-deepseekgui="memory-reader">
            <MarkdownReader source={memory.text.slice(0, MEMORY_VIEW_MAX_CHARS)} />
              ...
              {hasAgents
                ...
                : (
                  <button
                    ...
                    onClick={() => { void bridge.run({ type: 'create-project-agents', sessionId }).then(() => setAgentsCreated(true)).catch(() => undefined) }}
                  >
