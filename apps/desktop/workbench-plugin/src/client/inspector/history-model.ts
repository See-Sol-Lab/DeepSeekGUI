...
export function toolRootOf(node: { readonly kind: string; readonly data: unknown }): ToolCallBlock | null {
...
/** Whether any in-window tool result could still be running. */
export function hasRunning(entries: readonly HistoryToolEntry[]): boolean {
  return entries.some(entry => entry.state === 'running')
}
