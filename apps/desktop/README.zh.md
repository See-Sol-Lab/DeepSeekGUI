...
| 交付身份 | About 对话框与所有工件携带四个来自唯一来源的事实：DeepSeekGUI app version（`apps/desktop/package.json`，B5 开发态候选为 `1.1.0`，独立于 DSH 仓库版本）、从实际打包 Runtime 读取的 embedded DSH version、embedded DSH source/commit 标识，以及 Electron version + platform/arch——由 `src/version-info.ts` 组装，每个事实只有一个权威来源，不一致时构建门禁明确失败。 |
