...
flowchart LR
  ...
  pkg_workbench_inspector["workbench-inspector"]
  svc_workbenchInspector["ctx.workbenchInspector<br/>Read-only Workbench inspection"]
  ...
  pkg_git["git"]
  svc_git["ctx.git<br/>Git capability seam"]
  pkg_git_local["git-local"]
  pkg_pull_request["pull-request"]
  svc_pullRequest["ctx.pullRequest<br/>Pull request capability seam"]
  pkg_pull_request_gh["pull-request-gh"]
  ...
  pkg_pull_request --> svc_pullRequest
  pkg_pull_request_gh --> svc_pullRequest
  ...
  pkg_workbench_inspector --> svc_workbenchInspector
...
| `ctx.workbenchInspector` | `core` | [`workbench-inspector`](../packages/api/workbench-inspector) | - | - | - | 经现有提供方读取当前文件系统和 Git 事实，以 SessionQuery 定位目录，不执行模型。 |
...
| `ctx.git` | `seam` | [`git`](../packages/git/git) | [`git-local`](../packages/git/git-local) | - | - | 经稳定机器格式提供只读仓库事实；worktreeAdd/worktreeRemove 是仅有的 worktree 写入，预留给用户或 agent 明确选择的 worktree 动作。 |
| `ctx.pullRequest` | `seam` | [`pull-request`](../packages/pull-request/pull-request) | [`pull-request-gh`](../packages/pull-request/pull-request-gh) | - | - | 通过既有 provider 登录创建 pull request——绝不用 token；首个本地 provider 以 exact argv 调用用户已登录的 gh。 |
