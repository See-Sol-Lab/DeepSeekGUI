...
| `ctx.workbenchInspector` | `core` | [`workbench-inspector`](../packages/api/workbench-inspector) | - | - | - | Reads current filesystem and Git facts through existing providers, anchored by SessionQuery without model execution. |
...
| `ctx.git` | `seam` | [`git`](../packages/git/git) | [`git-local`](../packages/git/git-local) | - | - | Read-only repository facts through stable machine formats; worktreeAdd/worktreeRemove are the only worktree writes, reserved for explicit user- or agent-chosen worktree actions. |
| `ctx.pullRequest` | `seam` | [`pull-request`](../packages/pull-request/pull-request) | [`pull-request-gh`](../packages/pull-request/pull-request-gh) | - | - | Create pull requests through an existing provider login — never a token; the first local provider calls the user’s logged-in gh with exact argv. |
