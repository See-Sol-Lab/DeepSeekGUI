...
## `SessionDeletionRecord` — 最小删除元数据

可选的 `SessionPersistence.deletions` 能力保留 `sessionId`、可为空的 `title`、`deletedAt`（Unix 毫秒）、用于工作区授权的可选 `cwd`，以及 `state`（`deleting` 或 `deleted`）。记录不包含消息或附件正文。`remove` 要求请求已获授权，Agent 已结束工作并释放；未完成的清理可以重试。共享附件字节不属于本次删除。
