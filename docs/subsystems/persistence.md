...
## `SessionDeletionRecord` — minimal deletion metadata

The optional `SessionPersistence.deletions` capability retains `sessionId`, nullable `title`, `deletedAt` (Unix milliseconds), optional `cwd` for workspace authorization, and `state` (`deleting` or `deleted`). It stores no message or attachment content. `remove` requires an authorized request and an Agent that has already drained and detached; incomplete cleanup remains retryable. Shared attachment bytes are outside this deletion.
