...
本指南介绍「改动」「Git」「并行工作区」三个视图和专用 Git / PR 工具。「记忆」视图见[单独的指南](memory.zh.md)。
...
![会话旁边的改动视图](assets/workbench-light-1.1.0.png)
...
![Git 视图中的分支、远端与会话提交记录](assets/git-1.1.0.png)

## 并行工作区视图

并行工作区视图列出仓库已注册的 Git worktree，以及各自的分支和改动摘要。多个 worktree 修改同一路径时，视图会标出重叠，方便在冲突发生之前处理。
...
每个改变仓库状态的 Git 与 PR 操作都经过标准 Harness 审批。请求会写明具体操作和目标，结果以专用卡片在会话中展示，摘要可展开查看详情。
