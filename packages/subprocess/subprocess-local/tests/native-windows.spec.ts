...
describe.skipIf(!windowsNative)('Windows Job native containment', () => {
  ...
  it('preserves missing-target and invalid-executable rejection errors', async () => {
    // A bare relative file name only resolves against the target cwd when
    // NoDefaultCurrentDirectoryInExePath is absent, and that decision reads the
    // LAUNCHING process's environment, not the spec's. A workstation with the
    // Windows hardening variable set would fail the relative case, so it is
    // cleared for this case and restored afterwards.
    const hardening = process.env.NoDefaultCurrentDirectoryInExePath
    ...
    try {
      ...
      if (hardening !== undefined) process.env.NoDefaultCurrentDirectoryInExePath = hardening
    }
