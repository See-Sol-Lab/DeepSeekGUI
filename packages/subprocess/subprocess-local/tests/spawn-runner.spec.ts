...
describe('runner launch inputs', () => {
  ...
  it('resolves Windows executables with target-cwd and PATH search semantics', () => {
    ...
    // The current-directory root is only searched when NoDefaultCurrentDirectoryInExePath
    // is absent, and the parameter defaults to the real process environment. A machine
    // that sets that Windows hardening variable would otherwise fail every case below,
    // so the cases that assert the default search order pass an empty environment.
