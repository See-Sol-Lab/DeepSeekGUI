...
/**
 * The first `.css` the dockkit bundle fails on under this repository's layout.
 *
 * The bundle imports `@deepseek-ai/dsh-client-ui-primitives` by package name
 * before its own local stylesheet, and this sweep runs under tsx's ESM hook,
 * which resolves workspace package names through the `paths` in
 * tsconfig.base.json — to the primitives' `src`, whose barrel reaches
 * StateDot's stylesheet. The bundle's own `./components/dockkit.module.css` is
 * a relative specifier that is never reached, because the package-name import
 * precedes it and throws first.
 */
