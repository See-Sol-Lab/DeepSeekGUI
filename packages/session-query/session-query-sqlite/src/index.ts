...
export class SqliteSessionQueryEngine extends SessionQueryEngine {
  ...
  constructor(ctx: Context, config: Config) {
    ...
    ctx.on('session/content-deleting', id => this._serialized(undefined, async () => {
      // A previously opened search database can contain text even with lazy search disabled.
      ...
      try {
        ...
        try { db.exec('ROLLBACK') } catch (rollbackError) {
          throw new AggregateError([error, rollbackError], 'Session index deletion and rollback failed')
        }
        throw error
      }
