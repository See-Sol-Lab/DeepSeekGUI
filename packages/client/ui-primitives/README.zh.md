...
`SmoothedTextDisplay`（[`markdown/smooth-text.ts`](src/markdown/smooth-text.ts)）是产品端经 chat 视图可选 `chatTextDisplay` 服务组合进来的帧调度器：它以有界速率释放已收到正文块的显示前缀（`charsPerSecond`，每帧封顶，落后超过 `maxBacklogChars` 时向前跳，后续工具卡片因此永不等待），绝不切开代理对，并在流式结束后立即返回权威文本。它不持有任何 React 状态，时钟可注入，测试中调度完全确定。
