...
`SmoothedTextDisplay` ([`markdown/smooth-text.ts`](src/markdown/smooth-text.ts)) is the frame scheduler a product composes through the chat view's optional `chatTextDisplay` service. It releases an already-received block's display prefix at a bounded pace (`charsPerSecond`, capped per frame, jumping forward past `maxBacklogChars` so a following tool card never waits), never splits a surrogate pair, and returns the authoritative text as soon as streaming ends. It holds no React state and takes an injectable clock, so its schedule is deterministic in tests.
