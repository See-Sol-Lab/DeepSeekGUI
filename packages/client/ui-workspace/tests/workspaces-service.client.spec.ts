...
describe('UiWorkspaceService', () => {
  ...
  it('leaves a later panel selection in place when New Session finishes', async () => {
    ...
    // This fork returns the connect promise from startSession so a failed New
    // Session reaches its caller; this scenario resolves it below and awaits
    // openWorkspace instead.
    void b.uiWorkspace.startSession(wid('alpha'))
  ...
  it('targets an explicit, current-session, then recent Workspace and reports failed starts', async () => {
    ...
    // The failure reaches the caller now, on top of the log line: swallowing
    // it is what made a failed New Session look like a dead button
    // (莉莉丝 2026-09-10).
