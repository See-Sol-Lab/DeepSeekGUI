...
export function PermissionSelect({ value, locked, command, t }: PermissionSelectProps) {
  ...
  const items: MenuEntry[] = value.options
    ...
    .map((option) => {
      ...
      // The full-access row reads as a danger row (red label), so the risk is
      // visible before the confirmation dialog — the 0.1.1 presentation the
      // DeepSeekGUI acceptance asked to keep (2026-09-06).
      return {
        id: option.value,
        label: permissionLabel(option.value, option.name, t),
        ...icon === undefined ? {} : { icon },
        ...option.value === FULL_ACCESS ? { danger: true } : {},
      }
    })
