...
describe('Chat inject API', () => {
  ...
  it('smooths streaming prose only when a display service is composed (B6-P4)', async () => {
    const b = await bench()
    const { injected } = b.chatViewApi(ROOT)
    const input = { key: 'node-1:0', text: '你好，世界', streaming: true }

    // No smoother: the authoritative text paints directly and no tick runs.
    expect(injected.textDisplayActive()).toBe(false)
    expect(injected.displayText(input)).toBe('你好，世界')
    expect(() => { injected.releaseText(input.key) }).not.toThrow()

    const smoother = {
      display: vi.fn((request: typeof input) => request.text.slice(0, 2)),
      release: vi.fn(),
    }
    b.runtime.ctx.provide('chatTextDisplay', smoother as never)
    expect(injected.textDisplayActive()).toBe(true)
    expect(injected.displayText(input)).toBe('你好')
    expect(smoother.display).toHaveBeenCalledWith(input)
    injected.releaseText(input.key)
    expect(smoother.release).toHaveBeenCalledWith(input.key)
    await b.runtime.dispose()
  })
})
