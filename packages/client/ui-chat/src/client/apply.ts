    ...
    const disposeView = ctx.slots.register({
      ...
      inject: (sessionId: SessionId): ChatViewInjected => {
        ...
        return {
          ...
          // B6-P4: the optional prose smoother. Resolved per call so the
          // product plugin may load after this one; its absence paints the
          // authoritative text directly.
          displayText: input => ctx.get('chatTextDisplay')?.display(input) ?? input.text,
          releaseText: (key) => { ctx.get('chatTextDisplay')?.release(key) },
          textDisplayActive: () => ctx.get('chatTextDisplay') !== undefined,
