...
- [流式正文显示](#streaming-prose-display)
...
可选的 `chatTextDisplay` 服务让产品端决定每帧绘制多少已经收到的助手正文块。chat 视图每次调用时现取该服务——在此之后加载的插件同样生效——并在每个绘制的帧上对每个流式正文块调用 `display({ key, text, streaming })`；返回值始终是权威文本的前缀。只有当某个块显示的前缀仍短于其权威文本时才请求下一帧：模型在两段输出之间停顿时重绘随之停止，下一段到达时由其自身的 prop 变化恢复。Turn 结束、停止、报错或断连后，视图立即绘制权威文本并停止帧循环。只有正文被调度：思考、图片、工具行、审批、错误和用户消息保持原到达顺序，Session log、投影与 provider 请求都不受影响。没有该服务时视图直接绘制权威文本且不启动帧循环。块键由节点键加块索引组成，因此两行或两个 Session 永不共享调度；组件卸载时释放用过的全部键。
