...
- [Streaming prose display](#streaming-prose-display)
...
<a id="streaming-prose-display"></a>
## Streaming prose display

The optional `chatTextDisplay` service lets a product decide how much of an already-received assistant text block is painted on each frame. The chat view resolves it per call — a plugin composed after this one still counts — and calls `display({ key, text, streaming })` for every streaming text block on each painted frame; the returned string is always a prefix of the authoritative text. A frame is requested only while some block is still short of its authoritative text: a model that pauses between chunks stops the repaints, and the next chunk resumes them through its own prop change. Once the Turn settles, stops, errors, or loses its connection, the view paints the authoritative text immediately and stops ticking. Only prose is scheduled: reasoning, images, tool rows, approvals, errors, and user messages keep their exact arrival order, and the Session log, projection, and provider requests are untouched. Without the service the view paints authoritative text directly and starts no frame tick. Block keys are the node key plus the block index, so two rows or two Sessions never share a schedule, and the view releases every key it used on unmount.
