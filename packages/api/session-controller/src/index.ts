...
export class SessionController extends TypertRemoteService {
  ...
  /** Delete an ordinary Session after its current work drains; callable by authorized Host integrations.
   * @param sessionId - Session identity to remove.
   * @param signature - Host proof authorizing this Home and Session identity.
   * @returns Completion of content cleanup and the removal announcement.
   */
  async deleteSession(sessionId: SessionId, signature: string): Promise<void> {
    if (this.ctx.bail('session/deletion-authorize', sessionId, signature) !== true) {
      throw new Error('Desktop session deletion authorization is required')
    }
    try {
      await this.agents.deleteSession(sessionId, signature)
    } catch (error) {
      const rows = await this.listState.list().catch((refreshError: unknown) => {
        this.ctx.logger.warn(`Session deletion list refresh failed: ${String(refreshError)}`)
        return []
      })
      const row = rows.find(item => item.sessionId === sessionId)
      if (row !== undefined) this.ctx.emit('api-session/added', row)
      throw error
    }
  }
