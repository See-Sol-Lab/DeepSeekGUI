...
Deletion writes a durable incomplete record before clearing log generations. Failures can be retried; directory links are unlinked, the POSIX lock inode is retained, and shared attachment bytes remain. Deleted IDs are excluded from listings and cannot be reopened or written.
