...
import {
  ...
  type SessionDeletionRecord, type SessionDeletionStorage,
} from '@deepseek-ai/dsh-session-persistence'
import { assertDeletionPath, readDeletion, listDeletions, writeDeletion, removeSessionContent } from './deletions.ts'
...
class JsonlSessionPersistence extends SessionPersistence {
  override readonly deletions: SessionDeletionStorage = {
    get: id => readDeletion(this.root, id),
    list: () => listDeletions(this.root),
    remove: (id, title, cwd, authorization) => this.deleteContent(id, title, cwd, authorization),
  }
  ...
  async create(header: SessionHeader, options?: SessionPersistenceCreateOptions): Promise<SessionHandle> {
    ...
    // Snapshot before the first await: the caller may mutate its header
    // object right after calling, and the stored meta must not follow it.
    ...
    await this.assertNotDeleted(snapshot.id)
  ...
  async list(options?: SessionPersistenceListOptions): Promise<readonly SessionPersistenceSnapshot[]> {
    ...
    const deleted = new Set((await this.deletions.list()).map(record => record.sessionId))
    for (const artifact of await this.listArtifacts(signal)) {
      if (deleted.has(artifact.header.id)) continue
  ...
  async persistBatch(
    ...
    await this.assertNotDeleted(header.id)
  ...
  private async listArtifacts(signal?: AbortSignal): Promise<Array<{ header: SessionHeader; path: string }>> {
    ...
    const deletedNames = new Set((await this.deletions.list()).map(record => encodeSegment(record.sessionId)))
    for (const project of await this.listProjectDirs(signal)) {
      signal?.throwIfAborted()
      for (const dir of await this.listSessionDirs(project, signal)) {
        signal?.throwIfAborted()
        if (deletedNames.has(basename(dir))) continue
  ...
  private async assertNotDeleted(id: SessionId): Promise<void> {
    const record = await this.deletions.get(id)
    if (record !== undefined) throw new Error(`Session "${id}" ${record.state === 'deleted' ? 'has been deleted; its content is unavailable' : 'is being deleted; retry deletion to finish cleanup'}`)
  }

  private async deleteContent(
    id: SessionId, title: string | null, cwd: string | undefined, authorization: string,
  ): Promise<SessionDeletionRecord> {
    if (this.ctx.bail('session/deletion-authorize', id, authorization) !== true) throw new Error('Session deletion authorization is required')
    const previous = await this.deletions.get(id)
    if (previous?.state === 'deleted') return previous
    const selected = previous === undefined ? await this.findLog(id) : undefined
    // The caller proved the identity before detaching; an empty lazy Session may have no log after close.
    const record: SessionDeletionRecord = previous ?? {
      sessionId: id, title, ...(cwd === undefined ? {} : { cwd }), deletedAt: Date.now(), state: 'deleting',
    }
    const directory = selected === undefined ? sessionDir(this.root, record.cwd, id) : dirname(selected.sourcePath)
    await assertDeletionPath(this.root, directory)
    this.tracker.claimWrite(id)
    let lease: SessionWriteLease | undefined
    try {
      lease = await this.acquireLease(id, record.cwd, directory)
      await writeDeletion(this.root, record)
      this.coldLogMemo.delete(id)
      await this.ctx.serial('session/content-deleting', id)
      await assertDeletionPath(this.root, directory)
      await removeSessionContent(directory)
      const completed: SessionDeletionRecord = { ...record, state: 'deleted', deletedAt: Date.now() }
      await writeDeletion(this.root, completed)
      return completed
    } finally {
