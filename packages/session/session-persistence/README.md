...
Optional `deletions` stores minimal deletion records and removes content only after the Agent owner has drained and detached. The Host authorization event must explicitly allow the request; derived-content cleanup finishes before a completed record is published.
