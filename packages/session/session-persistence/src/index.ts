...
declare module '@deepseek-ai/cordis' {
  interface Events {
    /**
     * Require explicit Host approval for irreversible Session content deletion.
     * @mode bail
...
/** Metadata retained after content deletion; no message or attachment content. */
export interface SessionDeletionRecord {
  readonly sessionId: SessionId
  readonly cwd?: string
  readonly title: string | null
  readonly deletedAt: number
  /** Incomplete filesystem cleanup is retryable and is never reported as completed. */
  readonly state: 'deleting' | 'deleted'
}

/** Optional backend capability for irreversible content removal and durable deletion records. */
export interface SessionDeletionStorage {
  /** Read a deletion record by exact identity.
   * @param id - Session identity.
   * @returns Minimal metadata, or undefined if deletion has not started.
   */
  get(id: SessionId): Promise<SessionDeletionRecord | undefined>
  /** List metadata only; callers must apply workspace authorization before exposing it.
   * @returns Complete deletion records without message or attachment content.
   */
  list(): Promise<readonly SessionDeletionRecord[]>
  /** Remove content after its Agent owner has drained and released it; retry incomplete cleanup.
   * @param id - Identity whose existence the caller established before releasing it.
   * @param title - Last observed title, retained as deletion metadata.
   * @param cwd - Recorded workspace directory used for storage and access checks.
   * @param authorization - Host authorization bound to this Session and storage Home.
   * @returns Completed deletion metadata; failures leave retryable intent.
   */
  remove(id: SessionId, title: string | null, cwd: string | undefined, authorization: string): Promise<SessionDeletionRecord>
}
