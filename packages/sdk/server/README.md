...
The launcher appReady signal controls when stdio starts consuming requests, so a partially loaded or failed tree cannot initialize an agent. In-process embedders without that signal start immediately. The profile composition owns root-agent tools; input, output and exit remain runtime-only transport hooks for tests. The generated [configuration catalog](../../../docs/config-catalog.md#deepseek-aidsh-sdk-jsonrpc-server) documents accepted configuration.
