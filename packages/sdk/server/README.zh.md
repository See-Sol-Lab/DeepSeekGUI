...
启动器 appReady 控制 stdio 开始接收请求的时机，未完成或失败的插件树不能初始化 agent。没有该信号的进程内嵌入方式立即开始。Profile 组合拥有根 agent 工具，input、output 和 exit 是测试专用传输钩子。[配置目录](../../../docs/config-catalog.zh.md#deepseek-aidsh-sdk-jsonrpc-server)记录接受的配置。
