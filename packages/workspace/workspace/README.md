...
- **Archiving is reversible** — a hidden session keeps its history and its place, and `unarchiveSession` returns it to its Workspace group; the archive set is a durable display filter rather than a one-way move.
