...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  {
    key: 'git',
    summary: 'Abstract Git service.',
    description: 'Abstract Git service. Subclass, implement the methods, and load the subclass as a plugin — it registers as `ctx.git` (one implementation per context; loading a second throws, which is cordis\' standard duplicate-service behavior). Implementations must honor these semantics:\n\n- Every command runs the system git through the subprocess seam with exact executable + argv (never a shell string) and a bounded collected output.\n- Queries are read-only; the only writes are the B4-P3 worktree lifecycle (`worktreeAdd`/`worktreeRemove`) and the B4-P4 index/revert operations (`applyIndexPatch`, `stageFile`, `unstageFile`, `revertFile`). No commit, no push, no clean, no `reset --hard` anywhere.\n- Failures are classified only by verifiable facts: `GitUnavailableError` for resolution/spawn failures, `GitNotARepositoryError` when the repository probe fails, `GitUnsupportedVersionError` from `git --version` parsing, and `GitCommandFailedError` (with the raw stderr) for everything else.',
    methods: [
      {
        signature: 'abstract resolveGit(): Promise<string>',
        description: 'Resolve the git executable in this provider\'s execution world.',
        parameters: [],
        returns: 'the canonical executable path.',
      },
      {
        signature: 'abstract readonly minimumGitVersion: string',
        description: 'The minimum supported git version (porcelain v2 floor).',
        parameters: [],
      },
      {
        signature: 'abstract repoIdentity(cwd: string): Promise<RepoIdentity>',
        description: 'Confirm a working directory belongs to a repository and return its identity. A directory outside any repository rejects with GitNotARepositoryError.',
        parameters: [{ name: 'cwd', description: 'Working directory to probe.' }],
        returns: 'the repository root, git dir, and bare flag.',
      },
      {
        signature: 'abstract head(cwd: string): Promise<HeadState>',
        description: 'Where HEAD points in the repository containing `cwd`.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }],
        returns: 'the branch, detached, or unborn state.',
      },
      {
        signature: 'abstract upstream(cwd: string): Promise<UpstreamInfo | undefined>',
        description: 'The current branch\'s configured upstream, when one exists.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }],
        returns: 'the upstream ref with ahead/behind counts, or `undefined` when the branch has no upstream.',
      },
      {
        signature: 'abstract worktrees(cwd: string): Promise<WorktreeInfo[]>',
        description: 'Every work tree registered with the repository.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }],
        returns: 'the work-tree list in `git worktree list --porcelain` order.',
      },
      {
        signature: 'abstract worktreeAdd(cwd: string, path: string, branch: string): Promise<void>',
        description: 'Create a work tree with a new branch (`git worktree add -b <branch> <path>`). The path must not exist and must not already be registered; a branch name that already exists fails with `GitCommandFailedError` (the raw stderr names it). This is the ONLY work-tree write the seam offers.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }, { name: 'path', description: 'Absolute path for the new work tree.' }, { name: 'branch', description: 'New branch to create and check out there.' }],
        returns: 'resolution after the work tree exists.',
      },
      {
        signature: 'abstract worktreeRemove(cwd: string, path: string): Promise<void>',
        description: 'Remove a registered work tree (`git worktree remove <path>`). Git itself refuses a dirty work tree; the caller decides which checks run first.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }, { name: 'path', description: 'Absolute path of the work tree to remove.' }],
        returns: 'resolution after the work tree is removed.',
      },
      {
        signature: 'abstract status(cwd: string): Promise<RepoStatus>',
        description: 'The complete read-only work-tree state.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }],
        returns: 'clean/dirty facts and every changed path (staged, unstaged, untracked, and conflict entries).',
      },
      {
        signature: 'abstract log(cwd: string, max: number): Promise<CommitInfo[]>',
        description: 'The newest commits reachable from HEAD (`git log -n <max>`), read-only. Serves the Workbench Git view so a person can see what was committed without asking the model; an unborn branch yields an empty list.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }, { name: 'max', description: 'Upper bound on returned commits (at least 1).' }],
        returns: 'commits newest first.',
      },
      {
        signature: 'abstract diff( cwd: string, scope: DiffScope, path?: string, wantPatch?: boolean, patchMaxBytes?: number, ): Promise<DiffResult>',
        description: 'The read-only diff of one scope. `numstat` rows carry binary and line facts; a requested patch is refused whole with GitDiffTooLargeError when it exceeds `patchMaxBytes`. The patch is generated with `--binary`, so binary files carry a literal patch that `git apply` can consume — the same patch bytes the index operations re-apply.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }, { name: 'scope', description: '`\'staged\'` reads the index against HEAD, `\'unstaged\'` the work tree against the index.' }, { name: 'path', description: 'Optional pathspec limiting the diff to one path.' }, { name: 'wantPatch', description: 'When true, also return the unified patch for the requested scope/path.' }, { name: 'patchMaxBytes', description: 'Bound on the returned patch.' }],
        returns: 'the file summaries and the optional patch.',
      },
      {
        signature: 'abstract applyIndexPatch(cwd: string, patch: string, reverse: boolean): Promise<void>',
        description: 'Apply one patch to the INDEX only (B4-P4 hunk stage/unstage). The patch must be a git-diff fact (the caller hands back the exact bytes `diff` produced, or a hunk subset of them); it is delivered through the subprocess stdin, never via a shell string or a temp file. `reverse` applies the patch in reverse (unstage: index back toward HEAD). Git itself validates the context against the current index, so a stale hunk or an externally modified index fails with `GitCommandFailedError` and the raw stderr. Nothing else is written: the work tree is untouched.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }, { name: 'patch', description: 'The unified patch bytes (git-diff format).' }, { name: 'reverse', description: 'Apply the patch in reverse.' }],
        returns: 'resolution after the index was updated.',
      },
      {
        signature: 'abstract stageFile(cwd: string, path: string): Promise<void>',
        description: 'Stage one whole path into the index (`git add -- <path>`). Works for tracked modifications, renames, and untracked files alike; binary files need no special handling. Staging an untracked file is allowed — B4 only refuses to DELETE untracked files, never to track them.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }, { name: 'path', description: 'Path relative to the repo root.' }],
        returns: 'resolution after the index was updated.',
      },
      {
        signature: 'abstract unstageFile(cwd: string, path: string): Promise<void>',
        description: 'Unstage one whole path: the staged diff (index against HEAD, generated with `--binary`) is applied in reverse to the index, so the index moves back toward HEAD and the work tree is untouched. Works on unborn HEADs (a staged new file leaves the index again) and on binary and rename entries. A path with no staged changes is a no-op.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }, { name: 'path', description: 'Path relative to the repo root.' }],
        returns: 'resolution after the index was updated.',
      },
      {
        signature: 'abstract revertFile(cwd: string, path: string, expectedPatch?: string): Promise<void>',
        description: 'Controlled revert of one TRACKED path: the work-tree file is restored from the index (`git checkout -- <path>`), discarding its unstaged modifications; staged changes stay staged. Refused up front with GitRevertRefusedError for untracked paths (B4 never restores or deletes untracked files) and for unmerged conflict entries (B4 never auto-resolves conflicts); git itself refuses anything the precondition missed. The caller shows the exact target and the content that would be lost before invoking this.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }, { name: 'path', description: 'Path relative to the repo root.' }, { name: 'expectedPatch', description: 'Optional approved unstaged patch; changed content refuses the revert.' }],
        returns: 'resolution after the work tree was restored.',
      },
      {
        signature: 'abstract authorIdentity(cwd: string): Promise<AuthorIdentity | undefined>',
        description: 'The configured commit author (`git var GIT_AUTHOR_IDENT`), or `undefined` when no identity is configured (the command fails). This is the identity a commit would actually carry, including environment overrides.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }],
        returns: 'the name and email, or `undefined` when unconfigured.',
      },
      {
        signature: 'abstract stagedTree(cwd: string): Promise<string>',
        description: 'The index\'s current tree oid (`git write-tree`): the snapshot a caller confirms against. Writing one tree object is git\'s own normal behavior; the tree is never committed by this call.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }],
        returns: 'the tree oid.',
      },
      {
        signature: 'abstract commit(cwd: string, message: string, expectedTree: string): Promise<GitCommitResult>',
        description: 'Commit the staged tree with a user-confirmed message (B4-P5). Checks in this order, writing nothing on refusal: unresolved conflicts (`GitCommitRefusedError(\'conflict\')`), an empty index (`GitCommitRefusedError(\'empty-index\')`), a missing author identity (`GitCommitRefusedError(\'identity\')`), and staged-tree drift — the index tree must equal the `expectedTree` the caller\'s user confirmed (`GitStagedTreeDriftError`; the caller re-shows the staged diff and asks again). The message is delivered through stdin (`git commit -F -`), so any characters are safe and nothing is shell-interpreted. A hook refusal or other git failure surfaces as `GitCommandFailedError` with the raw stderr. After the commit, HEAD\'s tree is compared against `expectedTree` once more: the drift check closed the review window, and this comparison reports the millisecond window between the check and the commit itself — `treeMatchesExpected` is `false` only when that window was exploited.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }, { name: 'message', description: 'The commit message the user confirmed.' }, { name: 'expectedTree', description: 'The tree oid the user reviewed.' }],
        returns: 'the new commit\'s SHA plus whether its tree matches the reviewed tree.',
      },
      {
        signature: 'abstract remotes(cwd: string): Promise<RemoteInfo[]>',
        description: 'Every configured remote (`git remote -v`), in configuration order.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }],
        returns: 'the remote names with their fetch/push URLs.',
      },
      {
        signature: 'abstract pushPreview(cwd: string, remote: string, localBranch?: string, remoteBranch?: string): Promise<PushPreview>',
        description: 'The push preview (B4-P6): the remote URL, the local branch and its target remote ref, the commits that would reach the remote, whether the remote ref exists (queried with `git ls-remote`), the remote\'s default branch, and the configured credential helper — every fact the caller\'s user confirms before a push. The remote ref existence query can fail (offline, unknown host): the preview then carries the local facts and reports `remoteRefExists: undefined` rather than guessing.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }, { name: 'remote', description: 'The remote that would receive the push.' }, { name: 'localBranch', description: 'Source branch; omission selects the checked-out branch.' }, { name: 'remoteBranch', description: 'Target branch; omission uses the source branch name.' }],
        returns: 'the preview facts.',
        throws: ['{@link GitNoSuchRemoteError} when the remote is not configured.'],
      },
      {
        signature: 'abstract push( cwd: string, remote: string, localBranch: string, remoteBranch: string, expected?: PushApproval, ): Promise<PushOutcome>',
        description: 'Push one local branch to one remote branch (B4-P6): `git push <remote> <local>:<remoteBranch>`. This is an explicit external write — the caller shows the preview and obtains confirmation first. The upstream configuration is never touched by this call. Refusals classify by git\'s own stderr vocabulary: non-fast-forward, protected branch, auth, missing remote repository, network failure, or a generic rejection (`GitPushRefusedError`); nothing is retried.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }, { name: 'remote', description: 'The remote to push to.' }, { name: 'localBranch', description: 'The local branch to push.' }, { name: 'remoteBranch', description: 'The remote branch ref to update.' }, { name: 'expected', description: 'Approved source commit and effective URL; drift refuses before the write.' }],
        returns: 'the remote ref\'s new SHA.',
        throws: ['{@link GitNoSuchRemoteError} when the remote is not configured.', '{@link GitPushRefusedError} when the remote refuses the push.'],
      },
    ],
  },
  {
  ...
  {
    key: 'pullRequest',
    summary: 'Abstract pull-request service.',
    description: 'Abstract pull-request service. Subclass, implement the methods, and load the subclass as a plugin — it registers as `ctx.pullRequest` (one implementation per context; loading a second throws, which is cordis\' standard duplicate-service behavior). Implementations must honor these semantics:\n\n- Every command runs the provider executable through the subprocess seam with exact executable + argv (never a shell string).\n- Availability reports the provider\'s own login facts — host and account only; a token is never read, displayed, or persisted by this seam.\n- Creation is an explicit external write: the caller shows the full preview (title, body, base, head, draft) and obtains confirmation first.\n- Failures classify by the provider\'s own output: already-exists, auth, or other; nothing is retried, auto-reviewed, auto-labeled, or merged.',
    methods: [
      {
        signature: 'abstract availability(cwd: string): Promise<PullRequestAvailability>',
        description: 'Whether this environment can create pull requests: the provider executable exists and is logged in. The reported facts are host and account — never a token.',
        parameters: [{ name: 'cwd', description: 'Working directory (the repository the PR would target).' }],
        returns: 'availability with the login facts, or the missing precondition.',
      },
      {
        signature: 'abstract existing(cwd: string, head: string): Promise<ExistingPullRequest | undefined>',
        description: 'The existing pull request for a head branch, when one already exists — the duplicate-creation guard the caller shows before allowing a create.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }, { name: 'head', description: 'The head branch to look up.' }],
        returns: 'the existing PR, or `undefined` when none exists.',
      },
      {
        signature: 'abstract create( cwd: string, opts: { title: string; body: string; base: string; head: string; draft: boolean }, ): Promise<CreatedPullRequest>',
        description: 'Create one pull request with the user-confirmed title, body, base, and head, optionally as a draft. The caller shows the complete preview and obtains explicit confirmation before invoking this — a model suggestion is never an authorization. Failures classify as already-exists (the existing PR\'s URL is extracted), auth, or other, with the raw stderr.',
        parameters: [{ name: 'cwd', description: 'Working directory inside the repository.' }, { name: 'opts', description: 'The confirmed PR fields.' }],
        returns: 'the created PR\'s URL and number.',
        throws: ['{@link PullRequestUnavailableError} when the provider cannot run.', '{@link PullRequestFailedError} when creation fails.'],
      },
    ],
  },
  {
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'async deleteSession(sessionId: SessionId, signature: string): Promise<void>',
        description: 'Delete an ordinary Session after its current work drains; callable by authorized Host integrations.',
        parameters: [{ name: 'sessionId', description: 'Session identity to remove.' }, { name: 'signature', description: 'Host proof authorizing this Home and Session identity.' }],
  ...
  },
  {
    ...
    methods: [
      {
        signature: 'readonly deletions?: SessionDeletionStorage',
        description: 'Absent on backends which do not support deleting stored Sessions.',
        parameters: [],
      },
      {
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'async deletedSession(sessionId: SessionId): Promise<SessionDeletionRecord | undefined>',
        description: 'Read exact deletion metadata; callers retain responsibility for workspace authorization.',
        parameters: [{ name: 'sessionId', description: 'Exact Session identity.' }],
        returns: 'Its deletion record, if present.',
      },
      {
        signature: 'async deletedSessions(cwd: string, query: string, limit: number, signal?: AbortSignal): Promise<readonly SessionDeletionRecord[]>',
        description: 'Search deletion metadata by literal title or id inside one authorized workspace.',
        parameters: [{ name: 'cwd', description: 'Authorized workspace directory.' }, { name: 'query', description: 'Literal title or ID fragment.' }, { name: 'limit', description: 'Maximum number of returned records.' }, { name: 'signal', description: 'Optional cancellation.' }],
  ...
  },
  {
    ...
    summary: 'Read-only workspace inspection plus desktop-authorized Session deletion.',
    ...
    methods: [
      ...
      {
        signature: '@Remote async memory(sessionId: SessionId, signal: AbortSignal): Promise<WorkbenchMemory>',
        description: 'Read the project memory file (`<folder>.memory.md` in the Session cwd) and whether the project has an AGENTS.md. A missing memory file is a normal state ("no memory yet"), not a failure.',
      ...
      {
        signature: '@Remote async status(sessionId: SessionId, signal: AbortSignal): Promise<WorkbenchRepository>',
        description: 'Read current repository status without running a model or tool.',
        parameters: [{ name: 'sessionId', description: 'Session whose recorded cwd anchors the read.' }, { name: 'signal', description: 'Request cancellation.' }],
        returns: 'canonical Git status and root.',
      },
      {
        signature: '@Remote async diff(sessionId: SessionId, scope: DiffScope, path: string, signal: AbortSignal): Promise<DiffResult>',
        description: 'Read the current patch for one path or the complete selected scope.',
        parameters: [{ name: 'sessionId', description: 'Session whose recorded cwd anchors the read.' }, { name: 'scope', description: 'Index or unstaged changes.' }, { name: 'path', description: 'Repository-relative file; empty selects all changes.' }, { name: 'signal', description: 'Request cancellation.' }],
        returns: 'the bounded provider diff; truncation is never silent.',
      },
      {
        signature: '@Remote async lastReply(sessionId: SessionId, signal: AbortSignal): Promise<WorkbenchLastReply>',
        description: 'The newest assistant reply of a Session and whether its turn has ended. The desktop\'s feedback triage (#15) prompts a hidden session and polls this; reading the log through the query service keeps it off the streaming follow channel the desktop has no client for.',
        parameters: [{ name: 'sessionId', description: 'the session to read.' }, { name: 'signal', description: 'Request cancellation.' }],
      ...
      {
        signature: '@Remote async overview(sessionId: SessionId, signal: AbortSignal): Promise<WorkbenchOverview>',
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Restore one archived Session into its retained Workspace group.',
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Remove one session from the registry-global archive set durably. The session\'s accounting slot was retained by archiving, so it reappears in its original Workspace group — or the ungrouped bucket when it had none. An id not in the set resolves without writing.',
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  {
    name: 'session/content-deleting',
    mode: 'serial',
    signature: '\'session/content-deleting\'(sessionId: SessionId): Promise<void> | void',
    summary: 'Remove derived content after durable deletion intent, before content cleanup completes.',
    description: 'Remove derived content after durable deletion intent, before content cleanup completes.',
    parameters: [{ name: 'sessionId', description: 'Identity whose Agent has already drained and detached.' }],
  },
  {
  ...
  {
    name: 'session/deletion-authorize',
    mode: 'bail',
    signature: '\'session/deletion-authorize\'(sessionId: SessionId, authorization: string): boolean | undefined',
    summary: 'Require explicit Host approval for irreversible Session content deletion.',
    description: 'Require explicit Host approval for irreversible Session content deletion.',
    parameters: [{ name: 'sessionId', description: 'Exact logical identity being deleted.' }, { name: 'authorization', description: 'Opaque authorization supplied by the requesting Host.' }],
  },
  {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'AuthorIdentity',
    declaration: 'export interface AuthorIdentity {\n    name: string;\n    email: string;\n}',
  },
  {
  ...
  {
    name: 'CommitInfo',
    declaration: 'export interface CommitInfo {\n    sha: string;\n    subject: string;\n    author: string;\n    time: number;\n}',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export interface CreatedPullRequest {\n    url: string;\n    number: number;\n}',
  },
  {
  ...
  {
    name: 'DiffFileSummary',
    declaration: 'export interface DiffFileSummary {\n    path: string;\n    origPath?: string;\n    status: \'added\' | \'modified\' | \'deleted\' | \'renamed\' | \'copied\' | \'typechange\';\n    binary: boolean;\n    addedLines?: number;\n    deletedLines?: number;\n    score?: number;\n}',
  },
  {
    name: 'DiffResult',
  ...
  {
    name: 'DiffScope',
    declaration: 'export type DiffScope = \'staged\' | \'unstaged\';',
  },
  {
    name: 'DirectoryEntry',
  ...
  {
    name: 'ExistingPullRequest',
    declaration: 'export interface ExistingPullRequest {\n    url: string;\n    number: number;\n}',
  },
  {
  ...
  {
    name: 'HeadState',
    declaration: 'export type HeadState = {\n    kind: \'branch\';\n    branch: string;\n    oid: string;\n} | {\n    kind: \'detached\';\n    oid: string;\n} | {\n    kind: \'unborn\';\n    branch: string;\n};',
  },
  {
  ...
  {
    name: 'PullRequestAuth',
    declaration: 'export interface PullRequestAuth {\n    host: string;\n    account: string;\n}',
  },
  {
    name: 'PullRequestAvailability',
    declaration: 'export type PullRequestAvailability = {\n    available: true;\n    auth: PullRequestAuth;\n} | {\n    available: false;\n    reason: \'missing-gh\' | \'not-authenticated\';\n};',
  },
  {
    name: 'PushApproval',
    declaration: 'export interface PushApproval {\n    sourceOid: string;\n    destinationToken: string;\n}',
  },
  {
    name: 'PushOutcome',
    declaration: 'export interface PushOutcome {\n    remote: string;\n    remoteBranch: string;\n    pushedSha: string;\n}',
  },
  {
    name: 'PushPreview',
    declaration: 'export interface PushPreview {\n    destinationToken: string;\n    sourceOid: string;\n    remote: RemoteInfo;\n    localBranch: string;\n    remoteBranch: string;\n    upstream?: string;\n    aheadCommits: string[];\n    remoteRefExists: boolean | undefined;\n    defaultBranch?: string;\n    credentialHelper?: string;\n}',
  },
  {
    name: 'ReadFileLine',
  ...
  {
    name: 'RemoteInfo',
    declaration: 'export interface RemoteInfo {\n    name: string;\n    fetchUrl: string;\n    pushUrl?: string;\n}',
  },
  {
    name: 'ReplayEnvelope',
  ...
  {
    name: 'RepoIdentity',
    declaration: 'export interface RepoIdentity {\n    root: string;\n    gitDir: string;\n    bare: boolean;\n}',
  },
  {
    name: 'RepoStatus',
    declaration: 'export interface RepoStatus {\n    clean: boolean;\n    head: {\n        kind: \'branch\' | \'detached\' | \'unborn\';\n        name: string;\n        oid?: string;\n    };\n    upstream?: {\n        ref: string;\n        ahead: number;\n        behind: number;\n    };\n    entries: StatusEntry[];\n}',
  },
  {
    name: 'RequestContext',
  ...
  {
    name: 'StatusCode',
    declaration: 'export type StatusCode = \'.\' | \'A\' | \'M\' | \'D\' | \'R\' | \'C\' | \'U\' | \'T\' | \'?\' | \'AM\' | \'AD\' | \'AU\' | \'AA\' | \'DD\' | \'DU\' | \'UA\' | \'UD\' | \'UU\';',
  },
  {
    name: 'StatusEntry',
    declaration: 'export interface StatusEntry {\n    kind: \'staged\' | \'unstaged\' | \'untracked\' | \'conflict\';\n    path: string;\n    origPath?: string;\n    x: StatusCode;\n    y: StatusCode;\n    score?: string;\n}',
  },
  {
    name: 'StorageBackend',
  ...
  {
    name: 'UpstreamInfo',
    declaration: 'export interface UpstreamInfo {\n    ref: string;\n    ahead: number;\n    behind: number;\n}',
  },
  {
    name: 'UserMessage',
  ...
  },
  {
    ...
    declaration: 'export interface WorkbenchWorktree extends WorktreeInfo {\n    statusError?: string;\n    current: boolean;\n    changedPaths: string[];\n}',
  },
  {
  ...
  {
    name: 'WorktreeInfo',
    declaration: 'export interface WorktreeInfo {\n    path: string;\n    head: string;\n    branch?: string;\n    detached: boolean;\n    bare: boolean;\n}',
  },
]
