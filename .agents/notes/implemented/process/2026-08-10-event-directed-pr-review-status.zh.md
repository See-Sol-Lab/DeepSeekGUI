...
`issue-lifecycle.yml` 当时不订阅 `pull_request.ready_for_review`，两条事件命令也都不依赖该动作；该工作流已于 2026-09-10 从本 fork 移除——它的 project token 限定在上游的 owner 与仓库，在这里每次运行都停在取 token 那步失败，除了通知邮件不产生任何结果。[Issue 策略](../../../../.github/workflows/issue-policy.yml)保留 `ready_for_review`，因为人工提交的 PR 进入评审时，该工作流负责执行必需检查门禁。
