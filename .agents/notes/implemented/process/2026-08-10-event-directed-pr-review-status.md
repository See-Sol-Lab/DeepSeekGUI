...
`issue-lifecycle.yml` remained unsubscribed from `pull_request.ready_for_review`, and neither event command depended on that action; the workflow was removed from this fork on 2026-09-10 because its project token is scoped to the upstream owner and repository, so every run here failed at the token step and produced nothing but notification mail. [Issue policy](../../../../.github/workflows/issue-policy.yml) retains `ready_for_review` because it owns required-check enforcement when a human pull request enters review.
