...
SDK stdio 监听在启动器 appReady 信号到达时开始。若在组合过程中开始监听，initialize 可能在失败或被替换的插件树释放时到达，把实际装载错误掩盖成 inactive-context 错误。没有启动器的进程内测试保留立即启动行为，释放时注销就绪回调，不用计时器或重试模拟该信号。
