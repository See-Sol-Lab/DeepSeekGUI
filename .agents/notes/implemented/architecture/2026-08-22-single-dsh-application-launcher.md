...
The SDK stdio listener starts at the launcher's appReady signal. A listener started during composition can receive initialize while a failed or replaced plugin tree is being disposed, masking its load error as an inactive-context error. In-process tests without a launcher retain immediate startup. The readiness callback is removed on disposal; no timer or retry approximates this signal.
